...
prosodylab = {
  ...
  getMessages: function(language) {
    var allMessages = this.loadCSV(`prosodylab/messages.tsv`);
    var arrayLength = allMessages.length;
    var messages = {};
    for (var i = 0; i < arrayLength; i++){
      if (language==(allMessages[i].message)) {
        messages =
        {
          thankyou: allMessages[i].thankyou,
          fullScreen: allMessages[i].fullScreen,
          continueButton: allMessages[i].continueButton,
          completionCode: allMessages[i].completionCode,
          connectHeadphones: allMessages[i].connectHeadphones,
          playSound: allMessages[i].playSound,
          playAgain: allMessages[i].playAgain,
          proceed: allMessages[i].proceed,
          adjustVolume: allMessages[i].adjustVolume,
          soundCheckOk: allMessages[i].soundCheckOk,
          askReplay: allMessages[i].askReplay,
          recordSound: allMessages[i].recordSound,
          startRecording: allMessages[i].startRecording,
          recordDialogue: allMessages[i].recordDialogue,
          saySentence: allMessages[i].saySentence,
          doneRecording: allMessages[i].doneRecording,
          speakNow: allMessages[i].speakNow,
          recordAgain: allMessages[i].recordAgain,
          recordCheckOk: allMessages[i].recordCheckOk,
          adjustMic: allMessages[i].adjustMic,
          adjustMicAfter: allMessages[i].adjustMicAfter,
          productionTestSentence: allMessages[i].productionTestSentence,
          recordCheckNotOk: allMessages[i].recordCheckNotOk,
          instructionsHeadphoneCheck: allMessages[i].instructionsHeadphoneCheck,
          startHeadphoneTest: allMessages[i].startHeadphoneTest,
          questionHeadphoneTest: allMessages[i].questionHeadphoneTest,
          optionsHeadphoneTest: allMessages[i].optionsHeadphoneTest,
          rateScale: allMessages[i].rateScale,
          naturalnessQuestion: allMessages[i].naturalnessQuestion,
          naturalnessQuestionContext: allMessages[i].naturalnessQuestionContext,
          naturalnessOptions: allMessages[i].naturalnessOptions,
          mqTrainingTheory: allMessages[i].mqTrainingTheory,
          mqKnowTheory: allMessages[i].mqKnowTheory,
          mqTrainingPractice: allMessages[i].mqTrainingPractice,
          mqMakeProfessional: allMessages[i].mqMakeProfessional,
          mqMakePractice: allMessages[i].mqMakePractice,
          mqMakeHobby: allMessages[i].mqMakeHobby,
          scaleYears: allMessages[i].scaleYears,
          scaleHowMuch: allMessages[i].scaleHowMuch,
          scaleHowOften: allMessages[i].scaleHowOften,
        }
      }
    }
    return messages;
